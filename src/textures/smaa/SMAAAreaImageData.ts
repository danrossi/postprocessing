...
const orthogonalSubsamplingOffsets = [0.0, -0.25, 0.25, -0.125, 0.125, -0.375, 0.375];
...
function smoothArea(d: number): void {
 ...
 *
 * @param p1X - The starting point of the line, X-component.
 * @param p1Y - The starting point of the line, Y-component.
 * @param p2X - The ending point of the line, X-component.
 * @param p2Y - The ending point of the line, Y-component.
 * @param x - The pixel index.
 * @param result - A target tupel to store the area in.
 */

function getOrthArea(p1X: number, p1Y: number, p2X: number, p2Y: number, x: number, result: number[]): void {
...
function getOrthAreaForPattern(pattern: number, left: number, right: number, offset: number, result: number[]): void {
	...
	switch(pattern) {
		...
		case 1: {
			...
			//   |
			//
			// The offset is only applied to L patterns in the crossing edge side to make it converge with the unfiltered
			// pattern 0. The pattern 0 must not be filtered to avoid artifacts.
		...
		case 6: {
			...
			//          |
			//
			// A problem of not offseting L patterns (see above) is that for certain max search distances, the pixels in the
			// center of a Z pattern will detect the full Z pattern, while the pixels in the sides will detect an L pattern.
			// To avoid discontinuities, the full offsetted Z revectorization is blended with partially offsetted L patterns.
 ...
 *
 * @param a1X - The lower bounds of the area, X-component.
 * @param a1Y - The lower bounds of the area, Y-component.
 * @param a2X - The upper bounds of the area, X-component.
 * @param a2Y - The upper bounds of the area, Y-component.
 * @param x - The X-coordinate.
 * @param y - The Y-coordinate.
 * @return Whether the pixel lies inside the area.
 */

function isInsideArea(a1X: number, a1Y: number, a2X: number, a2Y: number, x: number, y: number): boolean {
...
function getDiagAreaForPixel(a1X: number, a1Y: number, a2X: number, a2Y: number, pX: number, pY: number): number {
...
function getDiagArea(pattern: number, a1X: number, a1Y: number, a2X: number, a2Y: number,
	left: number, offset: number[], result: number[]): void {
...
function getDiagAreaForPattern(pattern: number, left: number, right: number,
	offset: number[], result: number[]): void {
...
function generatePatterns(patterns: RawImageData[], offset: number[], orthogonal: boolean): void {
 ...
 *
 * @param baseX - The base X-position.
...
function assemble(baseX: number, baseY: number, patterns: RawImageData[], edges: number[][],
	size: number, orthogonal: boolean, target: RawImageData): void {
...
export abstract class SMAAAreaImageData {
	...
	static generate(): RawImageData {
		...
		const orthogonalPatterns: RawImageData[] = [];
		const diagonalPatterns: RawImageData[] = [];
